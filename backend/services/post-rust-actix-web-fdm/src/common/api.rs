...
pub fn get_auth_token(ctx: &Context) -> Result<Option<&[u8]>> {
    match ctx.request.headers().get(AUTHORIZATION) {
        None => Ok(None),
        Some(header_value) => 
            match header_value.as_bytes().strip_prefix(b"Bearer ") {
...
impl super::PostContent {
    pub(super) fn url_empty() -> ErrorBody {
        ErrorBody {
            error: ErrBody {
                error: "URL_EMPTY".into(),
                reason: "url is empty".to_string(),
                message: "url is empty".to_string(),
            }
        }
    }
    pub(super) fn url_too_long() -> ErrorBody {
        ErrorBody {
            error: ErrBody {
                error: "URL_TOO_LONG".into(),
                reason: "url is too long".to_string(),
                message: "url is too long".to_string(),
            }
        }
    }
    pub(super) fn url_invalid(err: url::ParseError) -> ErrorBody {
        ErrorBody {
            error: ErrBody {
                error: "URL_INVALID".into(),
                reason: format!("{}", err),
                message: format!("url is invalid: {}", err),
            }
        }
    }
    pub(super) fn post_empty() -> ErrorBody {
        ErrorBody {
            error: ErrBody {
                error: "POST_EMPTY".into(),
                reason: "post is empty".to_string(),
                message: "post is empty".to_string(),
            }
        }
    }
    pub(super) fn post_too_long() -> ErrorBody {
        ErrorBody {
            error: ErrBody {
                error: "POST_TOO_LONG".into(),
                reason: "post is too long".to_string(),
                message: "post is too long".to_string(),
            }
        }
    }
}
