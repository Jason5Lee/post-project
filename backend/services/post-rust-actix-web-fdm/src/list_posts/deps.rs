...
pub async fn workflow(deps: &Deps, Query { creator, condition, size }: Query) -> Result<Output> {
    ...
    let (mut post_condition_sql, order) = match condition {
        ...
        Condition::Before(before) => (iformat!("WHERE `" db::posts::CREATION_TIME "` < " before.utc), "DESC"),
        Condition::After(after) => (iformat!("WHERE `" db::posts::CREATION_TIME "` > " after.utc), "ASC"),
    };
    ...
    let posts_db_result: Vec<(u64, u64, u64, String)> = sqlx::query_as(&post_sql).bind(size.to_u32())
        .fetch_all(&deps.pool).await.map_err(handle_internal_error)?;
    ...
    *users_query.last_mut().unwrap() = b')';
    ...
    let user_id_to_name: HashMap<u64, UserName> =
        sqlx::query_as(&String::from_utf8(users_query).unwrap())
            ...
            .map(|r| -> Result<(u64, UserName)> {
                ...
                Ok((
                    id,
                    UserName::try_new(username).map_err(handle_invalid_value_in_db(db::USERS, db::users::USER_NAME.into(), id))?,
                ))
            })
            .try_collect()
            .await
    ...
    let posts = posts_db_result
        ...
        .map(|(id, creator, creation_time_utc, title)| {
            Ok(PostInfo {
                id: PostId(id),
                title: Title::try_new(title).map_err(handle_invalid_value_in_db(db::POSTS, db::posts::TITLE.into(), id))?,
                creator: user_id_to_name
                    ...
                    .ok_or_else(|| {
                        let value_err = (creator, ErrorBody {
                            error: ErrBody {
                        ...
                        handle_invalid_value_in_db(db::POSTS, db::posts::CREATOR.into(), id)(value_err)
                    })?,
                creation: Time {
