...
interface InvalidException {
    operator fun invoke(body: FailureBody, value: Any): Exception
}
...
class OnInvalidRespond(private val status: HttpStatusCode, private val prefix: String? = null) : InvalidException {
    override fun invoke(body: FailureBody, value: Any): Exception =
        if (prefix == null) {
            ...
            HttpException(status, FailureBody(
                error = body.error.copy(error = prefix + body.error.error)
            ))
        }
}

inline fun onInvalid(crossinline block: (FailureBody, Any) -> Exception): InvalidException =
    object : InvalidException {
        override fun invoke(body: FailureBody, value: Any): Exception = block(body, value)
    }
