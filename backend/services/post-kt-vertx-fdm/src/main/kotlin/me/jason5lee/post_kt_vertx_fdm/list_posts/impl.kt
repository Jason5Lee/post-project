...
class WorkflowImpl(private val deps: Deps) : Workflow(), ErrorsImpl {
    override suspend fun run(input: Query): List<Post> {
        val filter = Document()
        val creatorMap: MutableMap<ObjectId, me.jason5lee.post_kt_vertx_fdm.common.UserName> = mutableMapOf()
        if (input.creator != null) {
            ...
            creatorMap[creatorOid] = me.jason5lee.post_kt_vertx_fdm.common.UserName(creatorUserDoc.get("name"))
        ...
        if (creatorMap.isEmpty()) {
            ...
            for (creatorDoc in creatorDocs) {
                ...
                creatorMap[creatorId] = me.jason5lee.post_kt_vertx_fdm.common.UserName(creatorDoc.get("name"))
            }
        }
        ...
        return posts.map { postDoc ->
            ...
            Post(
                id = me.jason5lee.post_kt_vertx_fdm.common.PostId(Db.formatId(postDoc.id)),
                title = me.jason5lee.post_kt_vertx_fdm.common.Title(postDoc.get("title")),
                creator = Creator(
                    id = me.jason5lee.post_kt_vertx_fdm.common.UserId(Db.formatId(creatorId)),
                    name = creatorName,
                ),
                creationTime = me.jason5lee.post_kt_vertx_fdm.common.Time(postDoc.get("creationTime")),
            )
        }
    }
}
