...
val api = HttpApi(HttpMethod.Post, "/register") { ctx, workflow: Workflow ->
    ...
    val output = workflow.run(
        Command(
            userName = me.jason5lee.post_kt_vertx_fdm.common.newUserName(req.userName).onInvalidRespond(HttpStatusCode.UnprocessableEntity),
            password = me.jason5lee.post_kt_vertx_fdm.common.newPassword(req.password).onInvalidRespond(HttpStatusCode.UnprocessableEntity),
        )
    )
